document.addEventListener('DOMContentLoaded', () => {
    ...
    collectWalletForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Auto-refresh the system status dashboard to immediately show the new wallet
                fetchStatus(true);
            } else {
            ...
            if (response.ok) {
                ...
                if (data.wallets && data.wallets.length > 0) {
                    data.wallets.forEach(wallet => {
                        ...
                        addressSpan.className = 'wallet-address-text';
                        addressSpan.textContent = wallet;

                        const actionBtn = document.createElement('button');
                        actionBtn.className = 'wallet-action-btn';
                        actionBtn.type = 'button';
                        actionBtn.setAttribute('aria-label', `Copy wallet address ${wallet} and view profit events`);
                        actionBtn.setAttribute('title', 'Copy address and view profit events');
                        actionBtn.textContent = '📋 Use Wallet';
                        ...
                        actionBtn.addEventListener('click', async () => {
                            ...
                            try {
                                ...
                                actionBtn.setAttribute('aria-label', 'Wallet address copied to clipboard');
                                actionBtn.classList.add('success-state');
                                setTimeout(() => {
                                    actionBtn.textContent = originalText;
                                    actionBtn.setAttribute('aria-label', `Copy wallet address ${wallet} and view profit events`);
                                    actionBtn.classList.remove('success-state');
                                }, 1500);
                            } catch (err) {
                                console.error('Failed to copy text: ', err);
                            }

                            // Auto-populate Profit Events search input
                            profitWalletAddressInput.value = wallet;
                            profitWalletAddressInput.removeAttribute('aria-invalid');
                            profitWalletAddressInput.classList.remove('invalid-input');

                            // Auto-trigger Profit Events form submission
                            profitEventsForm.dispatchEvent(new Event('submit'));
                        });
                    ...
                    noWalletsLi.className = 'empty-state';
    ...
    const toggleAutoRefresh = () => {
        if (autoRefreshCb.checked) {
            // Activate visual and accessibility indicator
            if (refreshIndicator) {
                refreshIndicator.className = 'pulsing-dot';
                refreshIndicator.setAttribute('aria-label', 'Auto-refresh active');
            }
            ...
            if (refreshIndicator) {
                ...
                refreshIndicator.removeAttribute('aria-label');
            }
